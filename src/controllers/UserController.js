...
import { createUser, findUserByEmail, findUsers } from "../models/User.js";
...
export const update = async (req,res) =>{
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed update user", message: error.message });
  };
};
...
export const delUser = async (req,res) => {
  try {
    ...
    res
    .status(500)
    .json({ error: "Failed delete user", message: error.message });
  }
}
export const createUserPost = async (req, res) => {
  try {
    const { userData, postData } = req.body;
    const user = await createUser(userData, postData);
    res.status(201).json({ user });
  } catch (error) {
    res
      .status(500)
